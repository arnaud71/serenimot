...
const OUTPUT_PATH = process.argv[5] ?? "lexicon/generated/generated-3-letter-word-explanations.json";
const REPORT_PATH = process.argv[6] ?? "lexicon/generated/three-letter-word-explanations-report.json";
const WIKTIONARY_DEFINITIONS_PATH =
  process.argv[7] ?? "lexicon/generated/wiktionary-definitions.json";
...
const report = {
  ...
  mode: "three-letter-word-explanations",
  note: "Fiches courtes generees depuis les metadonnees Lexique. Les fiches revues manuellement restent prioritaires.",
  inputs: {
    words: WORDS_PATH,
  ...
  counts: {
    threeLetterWords: words.length,
    alreadyExplainedThreeLetterWords: words.length - generatedWords.length,
    generatedThreeLetterExplanations: generatedEntries.length,
    generatedWithWiktionaryDefinition: generatedEntries.filter((entry) =>
      entry.sources.some((source) => source.name === "Wiktionnaire")
    ).length,
    coveredThreeLetterWords: words.length
  }
};
...
console.log(`Generated ${generatedEntries.length} three-letter explanations -> ${OUTPUT_PATH}`);
  ...
  return omitEmpty({
    ...
    sources: hasWiktionaryDefinition
      ? [
          ...
          { name: "Wiktionnaire", version: "extraction locale" }
        ]
      : [{ name: "Lexique", version: "4.00" }],
    reviewed: false
  });
...
function compactLongDefinition(definition) {
  ...
  for (const separator of [" qui ", " dont ", " utilisé ", " composée ", " composé "]) {
...
function formatNounLabel(meta) {
  ...
  const genderLabel = genders.includes("f") && !genders.includes("m") ? " féminin" : genders.includes("m") && !genders.includes("f") ? " masculin" : "";
...
function formatDefinition(word, categories, lemma) {
  ...
  if (categories.includes("ART")) {
    return "Petit mot grammatical employé comme article.";
  }

  if (categories.includes("PRO")) {
    return "Petit mot grammatical employé comme pronom.";
  }

  if (categories.includes("PRE")) {
    return "Petit mot grammatical employé comme préposition.";
  }

  if (categories.includes("CON")) {
    return "Petit mot grammatical employé comme conjonction.";
  }

  if (categories.includes("ADV")) {
    return "Petit mot grammatical employé comme adverbe.";
  }

  if (categories.includes("ADJ")) {
    return "Adjectif court lexicalisé.";
  }

  if (categories.includes("NOM")) {
    return "Nom court lexicalisé.";
  }

  return "Mot court lexicalisé dans le lexique actif.";
}
